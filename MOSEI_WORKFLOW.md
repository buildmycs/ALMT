...
本轮实验将学习率从 `5e-5` 降至 `3e-5`；训练融合权重为 `0.45`，有序损失权重为
...
python scripts/bestweight.py \
  --predictions ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/best_validation_predictions.npz \
  --step 0.01 \
  --top-k 10
```
...
```text
ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/rho_search_validation/
├── rho_search_all.csv
...
以下保留 `0.95` 作为命令格式示例，它不是新实验的推荐值或已选定值。
实际运行时替换为本轮验证集搜索出的 rho，并同步调整输出目录名称。

```bash
python scripts/evaluate_selected_test.py \
  --config_file configs/mosei_dual_c4_intensity.yaml \
  --checkpoint ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/best_validation_model.pth \
  --ordinal-prediction-weight 0.95 \
  --output-dir ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/rho_095_test \
  --gpu_id 0
```
...
python scripts/analyze_acc7.py \
  --predictions ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/rho_095_test/selected_test_predictions.npz \
  --title "MOSEI test - validation-selected rho=0.95" \
  --output-dir ckpt/ALMT_MOSEI_Dual_C4_Intensity_rho045_ord020_fixed_e100_wu10_lr3e-5_seed0/rho_095_test/acc7
```
